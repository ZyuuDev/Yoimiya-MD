...
const {
  ...
  proto,
  Browsers
} = require('@rexxhayanasi/elaina-bail')
const fs = require('fs')
...
const jadibot = async (DinzBotz, m, from) => {
  if (Object.keys(client).includes(from)) {
    return DinzBotz.sendMessage(from, {
      text: `ᴋᴀᴍᴜ sᴜᴅᴀʜ ᴊᴀᴅɪʙᴏᴛ sᴇʙᴇʟᴜᴍɴʏᴀ!\n\n\`ᴜɴᴛᴜᴋ sᴛᴏᴘ ᴊᴀᴅɪ ʙᴏᴛ sɪʟᴀʜᴋᴀɴ ɢᴜɴᴀᴋᴀɴ ᴄᴏᴍᴍᴀɴᴅ [ .sᴛᴏᴘᴊᴀᴅɪʙᴏᴛ ]\``
    }, {
      quoted: m
    })
  }
    ...
    async function connectToWhatsApp() {
      ...
      if (usePairingCode && !client[from].user && !client[from].authState.creds.registered) {
        setTimeout(async () => {
          ...
          let txt = `*[ 𝗝𝗔𝗗𝗜𝗕𝗢𝗧 - 𝗖𝗟𝗢𝗡𝗘 ]*\nᴋᴏᴅᴇ ᴘᴀɪʀɪɴɢ ᴍᴜ: ${code}\n\nᴍᴀsᴜᴋᴀɴ ᴋᴏᴅᴇ ᴋᴇ ᴘᴇʀᴀɴɢᴋᴀᴛ ᴛᴇʀᴛᴀᴜᴛ\nᴜɴᴛᴜᴋ ᴍᴇɴᴊᴀᴅɪ ʙᴏᴛ\n\nHai sayang, kamu mau jadi bot ya kayak aku😋. aku yoimiya salam kenal🤩✨`
...
async function stopjadibot(DinzBotz, m, from) {
  if (!Object.keys(client).includes(from)) {
    return DinzBotz.sendMessage(from, {
      text: `> ᴋᴀᴍᴜ ᴛɪᴅᴀᴋ ᴀᴅᴀ ᴅɪ ʟɪsᴛ ᴊᴀᴅɪ ʙᴏᴛ`
    }, {
      quoted: m
    })
  }
...
async function listjadibot(DinzBotz, m) {
  let from = m.key.remoteJid
  let mentions = []
  let text = `\`_ʟɪsᴛ ᴊᴀᴅɪ ʙᴏᴛ_\`\n`
